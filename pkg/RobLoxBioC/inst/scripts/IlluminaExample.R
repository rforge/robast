...
allVar=cbind(NoBeads(bsd.sharpen.nobgc)*(se.exprs(bsd.sharpen.nobgc))^2,NoBeads(bsd.sharpen.subtract)*(se.exprs(bsd.sharpen.subtract))^2,NoBeads(bsd.sharpen.normexp)*(se.exprs(bsd.sharpen.normexp))^2)
...
boxplot(as.data.frame(allExprs[spikeInd,o]), col=c(rep(pal[1],4), rep(pal[2],4),rep(pal[3],4)), outline=FALSE,xlab="", ylab="Non-normalised log2 intensity",names=NULL, xaxt="n", ylim=c(4,16), main="A",font.main=2,medlwd=1)
...
axis(side=1,at=c(13,25,37,49,61,73,85,97,109,121,133,145)-6, labels=c("1000pM", "300pM", "100pM", "30pM", "10pM", "3pM", "1pM", "0.3pM", "0.1pM", "0.03pM", "0.01pM", "0pM"))
boxplot(as.data.frame(allVar[spikeInd,o]), col=c(rep(pal[1],4), rep(pal[2],4),rep(pal[3],4)), outline=FALSE,xlab="", ylab="Non-normalised log2 variance",names=NULL, xaxt="n", main="B", font.main=2,medlwd=1)
...
conts = makeContrasts(pm1000-pm300, pm300-pm100,pm100-pm30, pm30-pm10, pm10-pm3, pm3-pm1, pm1-pm0.3,pm0.3-pm0.1,pm0.3-pm0.1,pm0.1-pm0.03,pm0.03-pm0.01,levels=design)
...
plot(l1$coeff[spikeInd[1],], ylim=range(5.5,16), type="n",ylab="log2 intensity", xlab="Concentration",xaxt="n", cex=0.6, main="Non-normalised intensities of the spike genes")
...
axis(side=1,at=1:12, labels=c("1000pM", "300pM", "100pM", "30pM", "10pM", "3pM", "1pM", "0.3pM", "0.1pM", "0.03pM", "0.01pM", "0pM"))
dev.off()
...
########################################
## Fitting the linear model to separate background
## methods

##Make design matrix with 12 columns, one for each
##concentration
targets = read.table("spike_targets.txt",header=TRUE)
...
contr.1000 = makeContrasts(pm1000-pm300, pm1000-pm100, pm1000-pm30, pm1000-pm10, pm1000-pm3, pm1000-pm1, pm1000-pm0.3, pm1000-pm0.1, pm1000-pm0.03, pm1000-pm0.01, pm1000-pm0, levels=design)
contr.300 = makeContrasts(pm300-pm100, pm300-pm30, pm300-pm10, pm300-pm3, pm300-pm1, pm300-pm0.3, pm300-pm0.1, pm300-pm0.03, pm300-pm0.01, pm300-pm0, levels=design)
contr.100 = makeContrasts(pm100-pm30, pm100-pm10, pm100-pm3, pm100-pm1, pm100-pm0.3, pm100-pm0.1, pm100-pm0.03, pm100-pm0.01, pm100-pm0, levels=design)
contr.30 = makeContrasts(pm30-pm10, pm30-pm3, pm30-pm1, pm30-pm0.3, pm30-pm0.1, pm30-pm0.03, pm30-pm0.01, pm30-pm0, levels=design)
...
fit.pwts.subtract  = lmFit(normalizeQuantiles(exprs(bsd.sharpen.subtract)), design=design, weights=w.probes)
...
boxplot(as.data.frame(cont[spikeInd,]), main="A", ylim=c(-10,95), names=methods, las=2, ylab="log-odds", col=col,cex.lab=0.8)

boxplot(as.data.frame(cont[nonspikes,]), main="",  names=rep("",6), las=2, col=transcol, add=TRUE,pch="x")

cont=cbind(contr.sharpen.nobgc$coeff[,i],contr.pwts.nobgc$coeff[,i],contr.sharpen.subtract$coeff[,i], contr.pwts.subtract$coeff[,i],contr.sharpen.normexp$coeff[,i], contr.pwts.normexp$coeff[,i])
...
las=2,xlab="",names=controlInfo[s,2],col=pal[1], main="Distribution of intensities for 50 negative controls across all arrays")
...
## Figure 8 - Plotting array intensities in terms 
## of base composition
## quantile normalised, background corrected data is used

## Read annotation file and process sequences
## (stored in column 8)
...
M1Acount = apply(M1Ades, 1, sum)
M1Gcount = apply(M1Gdes, 1, sum)
M1Ccount = apply(M1Cdes, 1, sum)
M1Tcount = apply(M1Tdes, 1, sum)
...
par(mfrow=c(4,2))
boxplot(temp[M1oddstrips,1]~M1Acount[M1oddstrips], col=pal[1], outline=FALSE, ylim=c(4,11), main="Number of A bases", ylab="Normalised log2 intensity",varwidth=TRUE)
abline(h=6.38)
boxplot(temp[M1oddstrips,1]~M1Tcount[M1oddstrips], col=pal[1],outline=FALSE, ylim=c(4,11), main="Number of T bases", ylab="Normalised log2 intensity",varwidth=TRUE)
abline(h=6.38)
boxplot(temp[M1oddstrips,1]~M1Gcount[M1oddstrips], col=pal[1],outline=FALSE, ylim=c(4,11), main="Number of G bases", ylab="Normalised log2 intensity",varwidth=TRUE)
abline(h=6.38)
boxplot(temp[M1oddstrips,1]~M1Ccount[M1oddstrips], col=pal[1],outline=FALSE, ylim=c(4,11), main="Number of C bases", ylab="Normalised log2 intensity",varwidth=TRUE)
abline(h=6.38)
boxplot(temp[M1oddstrips,1]~M1GCcount[M1oddstrips], col=pal[1],outline=FALSE, ylim=c(4,11), main="GC count", ylab="Normalised log2 intensity",varwidth=TRUE)
abline(h=6.38)
boxplot(var[M1oddstrips]~M1GCcount[M1oddstrips], col=pal[1], outline=FALSE, main="GC Count", ylab="log2 variance",varwidth=TRUE)
abline(h=-2.17)
...
plot(1:50,l$coefficients[2:51], type="n", xlab="Base Position", main="Effect of base on log2 intensity relative to T",ylab="Effect Size",ylim=c(-0.4,0.4))
...
plot(1:50,l$coefficients[2:51], type="n",xlab="Base Position", main="Effect of base on log2 variance relative to T",ylim=c(-0.4,0.4),ylab="Effect Size")
text(1:50,l$coefficients[2:51], labels="A", col="red")
text(1:50,l$coefficients[52:101], labels="C", col="blue")
text(1:50,l$coefficients[102:151], labels="G", col="green")
abline(h=0)
...
boxplot(rk[M1oddstrips]~M1GCcount[M1oddstrips], col=pal[1],  ylab="Ranking based on log-odds", xlab="GC count", main="3pM - 1pM", varwidth=TRUE, outline=TRUE,pch="x") 
...
# thermodynamic properties calculated using Citation: Kibbe WA. 'OligoCalc: an online oligonucleotide properties calculator'. (2007) Nucleic Acids Res. 35(webserver issue): May 25.  
...
plot(cor(cbind(spikeTP[,2:13],spikeTP[,15:19]))[1:12,14],axes=F,xlab="concentration",ylab="correlation of intensity with ...",pch=16)
...
summary((cor(cbind(negconT[(negcon$olicode<100000000)&(negcon$groupID=="permuted_negative"),],log2(negperf[(negcon$olicode<100000000)&(negcon$groupID=="permuted_negative"),seq(3,97,2)]))))[4:51,1])
summary((cor(cbind(negconT[(negcon$olicode<100000000)&(negcon$groupID=="permuted_negative"),],log2(negperf[(negcon$olicode<100000000)&(negcon$groupID=="permuted_negative"),seq(3,97,2)]))))[4:51,2])
summary((cor(cbind(negconT[(negcon$olicode<100000000)&(negcon$groupID=="permuted_negative"),],log2(negperf[(negcon$olicode<100000000)&(negcon$groupID=="permuted_negative"),seq(3,97,2)]))))[4:51,3])
...
pdf("fdr.pdf")
for(i in 1:ncontr) {
  plot(sel, cumsum(fp[order(abs(contr.sharpen.nobgc$t)[,i], decreasing=TRUE)][sel]), xlab="Number selected", ylab="Number of false discoveries", main=paste(colnames(contr.all)[i], "fold-change"), type="l", col=col[1])
...
for(i in 1:narrays) {
  imageplot(bld.sharpen.nobgc, array=i, main=arraynms[i], log=TRUE, ncol=100, nrow=50, high="blue", low="white", whatToPlot="G", zlim=zlim)
  cat(i, " ")
}
dev.off()
...
par(mfrow=c(12,1), mai=c(0.3,0.1,0.3,0.1), oma=c(0,0,0,0))
for(i in 1:narrays) {
  imageplot(bld.sharpen.nobgc, array=i, main=arraynms[i], log=TRUE, ncol=100, nrow=50, high="blue", low="white", whatToPlot="Gb", zlim=zlim)
  cat(i, " ")
}
dev.off()
...
boxplotBeads(bld.sharpen.nobgc, names=seq(1:narrays), las=2, outline=FALSE, col=arraycols, main="Raw Foreground", what="G", ylim=ylim, ylab=expression(log[2](intensity)), medlwd=1)
dev.off()

png("boxplots.Gb.png", width=1024, height=640)
boxplotBeads(bld.sharpen.nobgc, names=seq(1:narrays), las=2, outline=FALSE, col=arraycols, main="Raw Background", what="Gb", ylim=ylim, ylab=expression(log[2](intensity)), medlwd=1)
dev.off()
...
medsig = apply(exprs(bsd.sharpen.subtract), 1, FUN="median", na.rm=TRUE)
...
boxplot(as.data.frame(exprs(bsd.sharpen.subtract)), names=seq(1:narrays), xlab="array", ylab=expression(log[2](intensity)), main="Summarised expression data per array", las=2, col=arraycols)
boxplot(as.data.frame(se.exprs(bsd.sharpen.subtract)), names=seq(1:narrays), xlab="array", ylab="Standard error", main="Standard errors per array", las=2, ylim=c(0,0.5), col=arraycols)
boxplot(as.data.frame(NoBeads(bsd.sharpen.subtract)), names=seq(1:narrays), xlab="array", ylab="Number of beads per bead type", main="Number of beads per array", las=2, col=arraycols)
dev.off()
...
# [1] RColorBrewer_1.0-1   gplots_2.3.2         gdata_2.3.1
# [4] gtools_2.4.0         sma_0.5.15           beadarray_1.6.0
# [7] affy_1.16.0          preprocessCore_1.0.0 affyio_1.6.0
#[10] geneplotter_1.16.0   lattice_0.16-5       annotate_1.16.0
#[13] xtable_1.5-2         AnnotationDbi_1.0.5  RSQLite_0.6-3
#[16] DBI_0.2-3            Biobase_1.16.0       limma_2.12.0
#
#loaded via a namespace (and not attached):
# [1] KernSmooth_2.22-21 grid_2.6.0         rcompgen_0.1-15