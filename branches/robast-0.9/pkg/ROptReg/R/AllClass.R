...
.onAttach <- function(library, pkg){
    # unlockBinding(".ROptRegOptions", asNamespace("ROptReg"))
    msga <- gettext(""
...
#    "Note that global options are controlled by distrModoptions() ---c.f. ?\"distrModoptions\"."
    ...
    buildStartupMessage(pkg = "ROptReg", msga, msgb,
...
setClass("RegTypeFamily", 
            ...
            prototype(name = "regression type family", 
                      ...
                      modifyParam = function(theta){LMCondDistribution()}, ### <- new !!! (not in thesis!)
                      props = character(0)),
...
setClass("L2RegTypeFamily",   ## taken from ROptRegTS
            representation(ErrorL2deriv = "EuclRandVarList",
                           ErrorL2deriv.fct = "function", ### new compared to ROptRegTS
            ...
            prototype(name = "L2 differentiable regression type family", 
                      ...
                      ErrorL2deriv.fct = function(theta) {f <- function(x) {x-theta}
                                                     return(f)},
...
setClass("GLMTypeFamily",
            representation(Link ="GLMLink"),
            contains = "RegTypeFamily")
# L2 differentiable regression type model
setClass("L2GLMTypeFamily",   ## taken from ROptRegTS
            representation(Link = "GLMlink"),
            prototype(name = "L2 differentiable regression type family",
                      distribution = LMCondDistribution(),
                      distrSymm = new("NoSymmetry"),
...
setClass("CondBdStWeight", representation(stand = "matrix"),
          prototype(stand = matrix(1)), contains = "CondBoundedWeight")
setClass("CondHampelWeight", representation(cent = "function"),
          prototype(cent = function(x) 0), contains = "CondBdStWeight")
...
# square integrable, conditionally centered (partial) Hampel-IC
setClass("CondHampelIC",
          prototype = prototype(name = "square integrable, conditionally centered (partial) IC",
                                  Curve = EuclRandVarList(EuclRandVariable(Map = list(function(x){x[1]*x[2]}),
                                                Domain = EuclideanSpace(dimension=2), Range = Reals())),
                                  Risks = list(),
